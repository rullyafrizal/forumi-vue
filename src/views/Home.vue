<template>
  <div class="flex flex-col min-h-screen justify-between bg-white">
    ...
    <div class="flex items-center justify-center pt-5">
      <form @submit.stop.prevent="fetchHomeData">
    ...
    <div class="flex items-center justify-center px-5 pb-10">
      <div @click="movePage(currentPage - 1)" class="hover:cursor-pointer inline-flex items-center py-2 px-4 mr-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
      ...
      <div>
        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Page {{currentPage}} of {{pagination.total_pages}} &nbsp;&nbsp;</p>
      </div>
      <div @click="movePage(currentPage + 1)" class="hover:cursor-pointer inline-flex items-center py-2 px-4 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
        Next
...
export default {
  ...
  computed: {
    ...
    ...mapState(useQueryParamStore, ['currentPage', 'search'])
  },
  methods: {
    ...mapActions(useQueryParamStore, ['setCurrentPage', 'setSearch']),
    async fetchHomeData () {
      ...
      try {
        ...
        if (res.status_code === 200 && res.message === 'success:get:questions') {
          ...
          this.setCurrentPage(this.pagination.current_page)
        }
      } catch (e) {
        loader.hide()
